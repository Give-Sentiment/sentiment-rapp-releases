...
JAR_SHA256="1315cbdac962e6df66d8c4273e3f187834fa738c981c944a53d08f461525328b"
...
# Two separate keystores so L0 and L1 get distinct libp2p peer IDs. Sharing one
# keystore makes both layers register with the relay under the same identity,
# which breaks Circuit Relay v2 routing (the relay can't disambiguate which
# layer the chain wants to reach back to).
KEYSTORE_L0="$WORKDIR/operator-l0.p12"
KEYSTORE_L1="$WORKDIR/operator-l1.p12"
...
# ── Detect this device's LAN/interface IP ───────────────────────────────────
# Used as SELF_HOST so libp2p advertises a real reachable address rather than
# /ip4/0.0.0.0/tcp/9003 (which dialers resolve to their own loopback). The
# libp2p relay + DCUtR hole-punching then completes inbound from public peers.
detect_self_ip() {
    local ip=""
    # macOS — primary interface (en0, then en1, then en2)
    ...
    # Linux — the source IP used to reach the public internet
    if [ -z "$ip" ] && command -v ip >/dev/null 2>&1; then
        ip=$(ip route get 8.8.8.8 2>/dev/null | awk '/src/{for(i=1;i<=NF;i++) if($i=="src") print $(i+1)}' | head -1)
    fi
    # Final fallback — hostname -I (Linux). Mac doesn't support -I but won't reach here.
...
ensure_keystore() {
    local gen=0
    for k in "$KEYSTORE_L0" "$KEYSTORE_L1"; do
        if [ ! -f "$k" ]; then
            log "Generating $(basename "$k")…"
            CL_KEYSTORE="$k" CL_KEYALIAS=alias CL_PASSWORD=password \
    ...
    if [ "$gen" = 1 ]; then
        ok "Operator keystores generated"
        warn "Both keystores are your node identity. Back them up if you intend to keep this node long-running."
    else
        ok "Operator keystores present"
    fi
}
...
ensure_config() {
    ...
    if [ -z "$self_ip" ]; then
        die "Couldn't auto-detect this device's IP. Set SELF_HOST manually in config.env."
    fi
    log "Detected device IP: $self_ip (used as SELF_HOST)"
...
# Node identity — your keystores are the source of truth. L0 and L1 use
# distinct keystores so they have distinct libp2p peer IDs (the relay routes
# by peer ID, so sharing one keystore breaks back-dial routing).
NODE_TYPE=validator
APP_IDENTIFIER=sentiment-reality
L0_KEYSTORE=$KEYSTORE_L0
L1_KEYSTORE=$KEYSTORE_L1

# Response encryption pubkey (responses your node receives are encrypted with this).
SENTIMENT_PUBLIC_KEY=$SENTIMENT_PUBLIC_KEY

# This device's reachable address. SELF_HOST is what your libp2p layer
# advertises to peers — it MUST be a real interface IP (auto-detected on
# first run), not 0.0.0.0 or 127.0.0.1, or the chain's libp2p dial-back
# loops to its own loopback.
SELF_HOST=$self_ip
# ADVERTISED_IP is what the SDK uses for HTTP cluster join. With libp2p as
# the inbound transport, this matches SELF_HOST.
ADVERTISED_IP=$self_ip
...
launch_node() {
    ...
    # NAT-traversal gate. The SDK reads CL_REQUIRES_LIBP2P as the default for
    # CoCellConfig.requiresLibp2p (combined/CoCellConfig.scala:26-27), which
    # flows to ClusterJoiner's selfRequiresLibp2p. When true, the joiner
    # pre-connects to the peer's libp2p endpoint outbound — establishing the
    # bidirectional libp2p connection that public peers reuse to reach back
    # (Libp2pHandshakeClient.hasConnectionFrom). Without this, the chain
    # falls through to the broken connectThroughRelay path and hits an NPE.
    # NOTE: the --l0/l1--requires-libp2p CLI flags control method.requiresLibp2p
    # (whether to BIND the p2p HTTP server) — a different field with the same
    # name. Both are needed for a fully NAT'd operator.
    export CL_REQUIRES_LIBP2P=true

    log "Starting Sentiment rApp node…"

    # Both L0 and L1 sync from the CHAIN's L0 (remote). The operator's own L0
    # is a follower-only consumer; L1 does its peer discovery against the
    # chain L0 (which has an active session because the chain writer ran it
    # as run-genesis). This is the same pattern the colocated Mac mini
    # cluster used — except validators there reached the chain L0 over
    # loopback/Tailscale, whereas here the operator uses its public IP.
    # Libp2p Circuit Relay v2 — relay.enabled is what Libp2pTransport actually
    # checks (TransportConfig.scala:94-100). The hole-punching.enabled and
    # nat-detection.enabled props exist in TransportConfig but no runtime
    # module reads them as of SDK build 1095, so we don't set them.
    nohup java --enable-native-access=ALL-UNNAMED \
        -Dtransport.libp2p.relay.enabled=true \
